...
- **Geolocation / "Locate me" silently fails**: browsers gate the geolocation API to secure contexts (HTTPS or `localhost` exactly). Reaching the site directly over the LAN address (e.g. `http://r730.local:5055/`) leaves geolocation unavailable. Use `https://soonstone.hexcaliper.com/` so Cloudflare's TLS termination satisfies the secure-context requirement.
