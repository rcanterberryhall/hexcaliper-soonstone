...
- PostgreSQL 16 (new database, not SQLite — time-series volume justifies it)
...
**Deployment:** fully containerized via `docker-compose`, following Hexcaliper's standard architecture. Two containers: the Flask app and a dedicated Postgres instance for this project. Routed through the existing Cloudflare Tunnel using the `setup_cloudflare.sh` conventions.
...
Postgres. UTC everywhere. Composite primary keys to make idempotent ingestion natural.

```sql
CREATE TABLE stations (
    station_id      TEXT PRIMARY KEY,           -- ICAO identifier, e.g. KOKC
    name            TEXT,
    ...
    taf_site        BOOLEAN DEFAULT FALSE,       -- does this station issue TAFs?
...
CREATE TABLE observations (
    ...
    altimeter_inhg      REAL,
    precip_1hr_in       REAL,
    present_weather     TEXT[],                  -- e.g. ['TSRA', 'BR']
    cloud_layers        JSONB,                   -- [{cover:'BKN', base_ft:3500}, ...]
...
CREATE TABLE tafs (
    ...
    parse_warnings      TEXT[],                  -- e.g. ['unrecognized_change_group']
    ingested_at         TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE taf_groups (
    taf_id              BIGINT NOT NULL REFERENCES tafs(taf_id) ON DELETE CASCADE,
    group_index         SMALLINT NOT NULL,       -- 0 = base, 1+ for change groups
    group_type          TEXT NOT NULL,           -- BASE, FM, BECMG, TEMPO, PROB30, PROB40
    group_from          TIMESTAMPTZ NOT NULL,
    group_to            TIMESTAMPTZ NOT NULL,
    probability_pct     SMALLINT,                -- 30, 40, or NULL
    wind_dir_deg        SMALLINT,
    wind_speed_kt       REAL,
    wind_gust_kt        REAL,
    visibility_sm       REAL,
...
- `present_weather` and `weather` are arrays because METAR/TAF can list multiple weather phenomena (e.g. `+TSRA BR FG`).
- `cloud_layers` is JSONB rather than a separate table because we always query layers together with the parent observation/group.
...
├── docker-compose.yml              # postgres + flask app
...
**v1 — NWS Public Forecast layer + LLM fallback parser.** Add NWS gridpoint forecast ingestion. Extends `forecasts` table with PoP, QPF, temp, dewpoint, sky cover. Requires a `points` lookup per station (cached). New popup row for "NWS public forecast" alongside the TAF row. Also implements the LLM fallback handler for TAF parse failures: failed parses fire a single LLM call to extract structured fields, row is inserted with `parse_method = 'llm_fallback'`. Rate-limited and toggleable via config.
...
- **Time zones:** UTC everywhere in the database and in API responses. Convert to local time only at render time in the frontend, using the browser's locale.
- **Idempotency:** every ingestion job must be safe to re-run. The composite primary keys handle this; do not add ON CONFLICT DO UPDATE that mutates existing rows except for the `last_seen` timestamp on stations.
...
- **Monitoring:** a `/health` endpoint that returns 200 if the most recent METAR ingest was within the last 90 minutes and the most recent TAF ingest was within the last 90 minutes. Cloudflare Tunnel can health-check this.
- **Database backups:** standard pg_dump nightly to the existing NAS path used by other Hexcaliper services.
...
2. Write `models.py` and the initial Alembic migration. Verify schema by running migrations against a local Postgres.
...
3. **Postgres deployment:** dedicated container, not a shared instance. Aligns with Hexcaliper's general containerized architecture and keeps this project isolated for backups and version upgrades.
